module.exports = function (bridge) {
	...
	function makeLog(filename) {
		return function log(text, sync, cb) {
			...
			if (!sync) {
				fs.appendFile(filename, text + '\n', function (err) {
					if (err && !cb) console.error(err.stack || err.message || err);
					if (cb) {
						cb(err);
					}
				});
	...
	function logMemoryUsage() {
		...
		memLog((new Date().toLocaleString()) + ' - ' + Object.keys(usage).map(function (key) {
			return key + ': ' + usage[key];
		}).join(', '));
	}

	if (config.debug.logMemoryUsage) {
		memLog = makeLog('memory-usage.log');
		setInterval(logMemoryUsage, 60000);
		memLog((new Date().toLocaleString()) + ' - program init');
		logMemoryUsage();
	} else {
		memLog = function nuzz() {};
	}
	...
	function doBroadcast(file, chatOrigin, title) {
		return function (cb) {
			...
			getFile(file).then(function (url) {
				...
				ffmpeg = spawn(require('ffmpeg-binaries').ffmpegPath(), [
					'-v', '-8',
					'-re',
					'-i', url,
				...
				if (e.message && e.message.match('ENOMEM')) {
					console.error('[Fatal error] potential memory leak, suiciding...');

					memLog((new Date().toLocaleString()) + ' - [CIRTICAL] bad exit due to - ' + e.stack || e.message || e, function () {
						process.exit(1);
					});
	...
	var queue = new Queue(config.loopSize, {
		JSONToTask: function (info) {
			...
			return {
				task: function (cb) {
					throw new Error(`not_implemented_type_${info.type}`);
				},
				info: info
			};
		},
	...
	queue.on('next', function (task, info) {
		fs.writeJsonSync('song_queue.json', queue, {
			space: 4
		});
