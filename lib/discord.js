	...
	bridge.allOnce([
		...
		function joinAudioChannel(channel) {
			...
			function cleanup() {
				...
				log(`${new Date().toLocaleTimeString()} reconnecting to discord server`);
			...
			var wrapper = new AbortStream(2 * 2 * 48000 * 8, 2 * 2 * 48000 * 4);
			// Be safe, wrap the stream before passing
			wrapper.on('abort_data', function () {
			...
			channel.join().then(function (connection) {
				...
				var proxiedMixer = mixer.pipe(wrapper);
