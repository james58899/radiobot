module.exports = function (bridge) {
	bridge.allOnce([
		...
		'volume',
		'mixer'
	], function (config, makeLog, URLHandlers, queue, doQueueSong, songList, addToSongList, doTTS, volume, mixer) {
		...
		var proxiedStream = new AbortStream(64 * 1000 * 8, 64 * 1000 * 4);
		// Be safe, wrap the stream before passing
		proxiedStream.on('abort_data', function () {
		...
		function joinAudioChannel(channel) {
			...
			channel.join().then(function (connection) {
				...
				dispatcher = connection.playConvertedStream(mixer.pipe(proxiedStream), config.discord.streamOptions);
		...
		client.once('ready', function () {
			...
			// broadcaster.playConvertedStream(proxiedOpusStream, {bitrate: 128000});
