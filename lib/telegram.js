module.exports = function (bridge) {
	...
	function (config, makeLog, URLHandlers, queue, doQueueSong, songList, addToSongList, doTTS, mixer, volume) {
		...
		handle(function (message) {
			...
			if(message.audio.file_size > 20480000) {
				bot.sendMessage(
					message.chat.id,
		...
		handle(function (message) {
			...
			if(message.document.file_size > 20480000) {
				bot.sendMessage(
					message.chat.id,
					"Oops! The file size seems larger than 20MB and I can't get it from Telegram.",
					{reply_to_message_id: message.message_id}
			...
			if (regex.test(message.document.file_name)) {
				URLHandlers.find('telegram:' + message.document.file_id).getInfo().then(function (info) {
		...
		URLHandlers.unshift({
			...
			getInfo: function (url) {
				return MediaInfo(getFile(url.replace(/^telegram:/, '')));
			},
