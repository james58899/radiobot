		...
		function (config, makeLog, URLHandlers, queue, doQueueSong, songList, addToSongList, doTTS, mixer, volume) {
			...
			function getFile(file) {
				...
				if (config.telegram.PWRTelegram) {
					return bot.getFile(file).then(file => new Promise(resolve => resolve(encodeURI('https://storage.pwrtelegram.xyz/' + file.file_path))));
				} else {
					return bot.getFileLink(file).then(url => new Promise(resolve => resolve(encodeURI(url))));
				}
			}
