module.exports = function (bridge) {
	...
	function (config, makeLog, URLHandlers, queue, doQueueSong, songList, addToSongList, doTTS, mixer, volume) {
		...
		handle(/\/start(\s|@|$)/, function (message) {
			bot.sendMessage(message.chat.id, config.telegram.startMessage, {reply_to_message_id: message.message_id}).catch(logTGErrorMessage);
		});
		...
		handle(/\/list(\s|@|$)/, function (message) {
			...
			bot.sendMessage(message.chat.id, output, {reply_to_message_id: message.message_id}).catch(logTGErrorMessage);
		});
		...
		handle(/\/skip(\s|@|$)/, function (message) {
			...
			queue.signal('stop');
			bot.sendMessage(message.chat.id, 'The current song will stop and be removed shortly.', {reply_to_message_id: message.message_id}).catch(logTGErrorMessage);
		});
		...
		handle(/\/next(\s|@|$)/, function (message) {
			...
			queue.signal('stop');
			bot.sendMessage(message.chat.id, 'The next song will start shortly.', {reply_to_message_id: message.message_id}).catch(logTGErrorMessage);
		});
		...
		handle(/\/tts(\s|@|$)/, function (message) {
			...
			if (speech) {
				...
				bot.sendMessage(message.chat.id, 'The text will be played shortly.', {reply_to_message_id: message.message_id}).catch(logTGErrorMessage);
			}
		});
		...
		handle(function (message) {
			...
			if (message.document.mime_type.macht(/audio/ig)) {
				...
				bot.sendMessage(message.chat.id, 'I don\'t know these formats.', {reply_to_message_id: message.message_id}).catch(logTGErrorMessage);
			}
		});
		...
		bridge.on('text_message', function (chatOrigin, text) {
			...
			bot.sendMessage(chatOrigin.chat_id, text, {reply_to_message_id: chatOrigin.message_id}).catch(logTGErrorMessage);
		});
